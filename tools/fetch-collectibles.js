...
const SHOP_IC = { sly: 'Sly', salubra: 'Salubra' };
...
(async () => {
  ...
  for (const [name, loc] of Object.entries(locs)) {
    ...
    rec.how = how;
    rec.ic = name;                              // ItemChanger's name: js/map.js's SPOTS place it
    const xy = coords(loc);
    if (xy) rec.xy = xy;
    out.push(rec);
  }
  for (const [kind, id, field, geo] of SHOPS) {
    ...
    out.push({ kind, scene: SHOP_ROOM[shop], id: slug(kind + '-' + id), how: ['pd', field], src: [shop, geo], ic: SHOP_IC[shop] });
  }
  ...
  const line = (r) => `    { id: ${q(r.id)}, kind: ${q(r.kind)}, scene: ${q(r.scene)}, how: [${r.how.map((x) => (typeof x === 'number' ? x : q(x))).join(', ')}]`
    ...
    + (r.xy ? `, xy: [${r.xy.join(', ')}]` : '') + (r.ic ? `, ic: ${q(r.ic)}` : '') + ' },';
